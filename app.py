...
# [NEW] Callback สำหรับการเปลี่ยนแท็บ
def set_active_tab():
    """อัปเดต session state เมื่อผู้ใช้เปลี่ยนแท็บ"""
    # Streamlit จะเก็บชื่อแท็บที่ถูกเลือกไว้ใน st.session_state[key]
    if 'main_tabs' in st.session_state:
        st.session_state.active_tab_label = st.session_state.main_tabs
...
# [NEW] Add state for active tab, default to Tab 2
...
# ตรวจสอบสถานะการแก้ไขเพื่อใช้ในการปิดการใช้งานแท็บอื่น (Req 2)
...
tab1, tab2, tab3 = st.tabs(
    tab_labels,
    key="main_tabs", # Key เพื่อติดตามแท็บที่ถูกเลือก
    on_change=set_active_tab, # ฟังก์ชันที่จะถูกเรียกเมื่อมีการเปลี่ยนแท็บด้วยตนเอง
    active_tab=st.session_state.active_tab_label # บังคับให้ Streamlit เปิดแท็บนี้
)
...
with tab2:
    ...
    if not edited_df.equals(display_df):
        ...
        # [NEW FIX]: เมื่อมีการแก้ไขในตาราง ให้ตั้งค่าแท็บกลับไปที่ 2 
        # เพื่อป้องกันการเด้งไปแท็บ 1 ในกรณีที่ Streamlit RERUN ตัวเอง
        st.session_state.active_tab_label = "2. บันทึกขั้นตอนการทำงาน"
    ...
    def save_log_data_callback():
        ...
        if response and response.get('status') == 'success':
            ...
            # [NEW FIX]: บังคับให้ Streamlit กลับมาที่แท็บ 2 หลังจากการบันทึกและ RERUN
            st.session_state.active_tab_label = "2. บันทึกขั้นตอนการทำงาน" 
            ...
            st.session_state.edited_log = True
            # [NEW FIX]: หากล้มเหลว ให้แท็บยังอยู่ที่ 2
            st.session_state.active_tab_label = "2. บันทึกขั้นตอนการทำงาน" 
...
with tab1:
    ...
    # Req 2: ป้องกันการใช้งานหากมีการแก้ไข
    if disabled_state:
